...
import pt.isel.mpd.monitrgw.service4.MonitrServiceAsyncNio;
...
*/
    ...
    private static void testMonitrSequential() throws Exception {
    ...
    private static void testMonitrLazy() throws Exception {
    ...
    private static void testMonitrAsync() throws Exception {
    ...
    private static void testMonitrAsyncNio() throws Exception {
        try(MonitrServiceAsyncNio nio = new MonitrServiceAsyncNio()) {
            IMonitrMarketData news3 = duration("Get news Async: ",
                    () -> nio
                        .GetLastNews() // 1 http request GetLasNews // Future<Stream<IMonitrMarketData>>
                        .thenApply(s -> s.findFirst().get()) //Future<IMonitrMarketData>
                        .get()); // 1 http request MonitrStockDetails --> 1 http request MonitrStockAnalysis
            delay(500L);
            System.out.println("Doing stuff....");
            duration("Get stock details 1: ", () -> news3.getStockDetails()); // 0 request
    ...
    public static void duration(String label, Runnable action) throws Exception {
        duration(label, () -> { action.run(); return null; });
    }

    public static <T > T duration(String label, SupplierThrowable<T> action) throws Exception {
